...
CONTAINS
  ...
  SUBROUTINE CLOUDSC_DRIVER( &
     ...
     & IPA,       PCLV,     IPSUPSAT,&
     & IPLCRIT_AER,IPICRIT_AER, IPRE_ICE, &
     & IPCCN,     IPNICE,&
     ...
     & YDOMCST, YDOETHF, YDECLDP, ZIFG, ZOFG)
   ...
   ! REAL(KIND=JPRB),    INTENT(OUT)   :: PFSQRF(:,:,:)  ! Flux diagnostics
    ...
    REAL(KIND=JPRB), INTENT(IN)   :: ZIFG (:,:,:,:)
    REAL(KIND=JPRB), INTENT(OUT)   :: ZOFG (:,:,:,:)
    ...
    DO JKGLO=1,NGPTOT,NPROMA
       ...
       ICEND=MIN(NPROMA,NGPTOT-JKGLO+1)
         ...
         CALL CLOUDSC &
              & (    1,    ICEND,    NPROMA,  NLEV,&
              & PTSPHY,&
              & IPT, IPQ, TENDENCY_CML(IBL), TENDENCY_TMP(IBL), TENDENCY_LOC(IBL), &
              & IPVFA, IPVFL, IPVFI, IPDYNA, IPDYNL, IPDYNI, &
              & IPHRSW,    IPHRLW,&
              & IPVERVEL,  IPAP,      IPAPH,&
              & PLSM(:,IBL),       LDCUM(:,IBL),      KTYPE(:,IBL), &
              & IPLU,      PLUDE(:,:,IBL),    IPSNDE,    IPMFU,     IPMFD,&
              !---prognostic fields
              & IPA,       PCLV(:,:,:,IBL),   IPSUPSAT,&
              ...
              & IPFPLSL,   IPFPLSN,   IPFHPSL,  IPFHPSN,&
              & KFLDX, &
              & YDOMCST, YDOETHF, YDECLDP, ZIFG(:,:,:,IBL), ZOFG(:,:,:,IBL))
