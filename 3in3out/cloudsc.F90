...
SUBROUTINE CLOUDSC &
 ...
 & PTSPHY,&
 & IPT, IPQ, tendency_cml,tendency_tmp,tendency_loc, &
 & IPVFA, IPVFL, IPVFI, IPDYNA, IPDYNL, IPDYNI, &
 & IPHRSW,    IPHRLW,&
 & IPVERVEL,  IPAP,      IPAPH,&
 & PLSM,     LDCUM,    KTYPE, &
 & IPLU,      PLUDE,    IPSNDE,    IPMFU,     IPMFD,&
 !---prognostic fields
 & IPA,&
 & PCLV,  &
 & IPSUPSAT,&
...
!!! & PQAER,    KAER, &
 & IPLCRIT_AER,IPICRIT_AER,&
 & IPRE_ICE,&
 & IPCCN,     IPNICE,&
 !---diagnostic output
 & PCOVPTOT, PRAINFRAC_TOPRFZ,&
 !---resulting fluxes
 & IPFSQLF,   IPFSQIF ,  IPFCQNNG,  IPFCQLNG,&
 & IPFSQRF,   IPFSQSF ,  IPFCQRNG,  IPFCQSNG,&
 & IPFSQLTUR, IPFSQITUR , &
 & IPFPLSL,   IPFPLSN,   IPFHPSL,   IPFHPSN, KFLDX, &
 & YDCST, YDTHF, YDECLDP, ZIFG, ZOFG)
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: ZIFG (KLON,KLEV+1,26)
REAL(KIND=JPRB)   ,INTENT(OUT)    :: ZOFG (KLON,KLEV+1,17)

INTEGER(KIND=JPIM),    INTENT(IN)    ::IPLCRIT_AER
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPICRIT_AER
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPRE_ICE
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPCCN
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPNICE


INTEGER(KIND=JPIM),    INTENT(IN)    ::IPT
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPQ

INTEGER(KIND=JPIM),    INTENT(IN)    ::IPVFA   !!!
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPVFL
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPVFI
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPDYNA   !!!
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPDYNL    !!!
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPDYNI   !!!
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPHRSW
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPHRLW
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPVERVEL
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPAP
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPAPH
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPLU
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPSNDE
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPMFU
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPMFD
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPA
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPSUPSAT





INTEGER(KIND=JPIM),    INTENT(IN)    :: IPFSQLF
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPFSQIF
INTEGER(KIND=JPIM),    INTENT(IN)    :: IPFCQLNG
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPFCQNNG
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPFSQRF
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPFSQSF
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPFCQRNG
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPFCQSNG
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPFSQLTUR
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPFSQITUR
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPFPLSL
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPFPLSN
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPFHPSL
INTEGER(KIND=JPIM),    INTENT(IN)    ::IPFHPSN
...
DO JK=1,KLEV
  DO JL=KIDIA,KFDIA
    ZTP1(JL,JK)        = ZIFG(JL,JK,IPT)+PTSPHY*tendency_tmp%T(JL,JK)
    ZQX(JL,JK,NCLDQV)  = ZIFG(JL,JK,IPQ)+PTSPHY*tendency_tmp%q(JL,JK) 
    ZQX0(JL,JK,NCLDQV) = ZIFG(JL,JK,IPQ)+PTSPHY*tendency_tmp%q(JL,JK)
    ZA(JL,JK)          = ZIFG(JL,JK,IPA)+PTSPHY*tendency_tmp%a(JL,JK)
    ZAORIG(JL,JK)      = ZIFG(JL,JK,IPA)+PTSPHY*tendency_tmp%a(JL,JK)
  ENDDO
ENDDO
...
DO JK=1,KLEV
  DO JL=KIDIA,KFDIA
    ...
    ZFOEEWMT(JL,JK)=MIN(FOEEWM(ZTP1(JL,JK))/ZIFG(JL,JK,IPAP),0.5_JPRB)
    ...
    ZFOEEW(JL,JK)=MIN((ZALFA*FOEELIQ(ZTP1(JL,JK))+ &
         &  (1.0_JPRB-ZALFA)*FOEEICE(ZTP1(JL,JK)))/ZIFG(JL,JK,IPAP),0.5_JPRB)
    ...
    !---------------------------------- 
    ZFOEELIQT(JL,JK)=MIN(FOEELIQ(ZTP1(JL,JK))/ZIFG(JL,JK,IPAP),0.5_JPRB)
...
DO JL=KIDIA,KFDIA
  ZTRPAUS(JL)=0.1_JPRB
  ZPAPHD(JL)=1.0_JPRB/ZIFG(JL,KLEV+1,IPAPH)
ENDDO
DO JK=1,KLEV-1
  DO JL=KIDIA,KFDIA
    ZSIG=ZIFG(JL,JK,IPAP)*ZPAPHD(JL)
...
!----------------------------------------------------------------------
 ...
 ! call ftrace_region_begin("line926") 
  DO JL=KIDIA,KFDIA
    ...
    ZDP(JL)     = ZIFG(JL,JK+1,IPAPH)-ZIFG(JL,JK,IPAPH)     ! dp
    ZGDP(JL)    = RG/ZDP(JL)                    ! g/dp
    ZRHO(JL)    = ZIFG(JL,JK,IPAP)/(RD*ZTP1(JL,JK))   ! p/RT air density

    ZDTGDP(JL)  = PTSPHY*ZGDP(JL)               ! dt g/dp
    ZRDTGDP(JL) = ZDP(JL)*(1.0_JPRB/(PTSPHY*RG))  ! 1/(dt g/dp)

    IF (JK>1) ZDTGDPF(JL) = PTSPHY*RG/(ZIFG(JL,JK,IPAP)-ZIFG(JL,JK-1,IPAP))
...
!DIR$ NOFUSION
  ...
  DO JL=KIDIA,KFDIA
   ...
   ENDIF
    ...
    !-------------------------------------------------------    
      IF (ZIFG(JL,JK,IPSUPSAT)>ZEPSEC) THEN
        IF (ZTP1(JL,JK) > RTHOMO) THEN
          ! Turn supersaturation into liquid water
          ZSOLQA(JL,NCLDQL,NCLDQL) = ZSOLQA(JL,NCLDQL,NCLDQL)+ZIFG(JL,JK,IPSUPSAT)
          ZPSUPSATSRCE(JL,NCLDQL) = ZIFG(JL,JK,IPSUPSAT)
          ! Add liquid to first guess for deposition term 
          ZQXFG(JL,NCLDQL)=ZQXFG(JL,NCLDQL)+ZIFG(JL,JK,IPSUPSAT)
          ! Store cloud budget diagnostics if required
        ELSE
          ! Turn supersaturation into ice water
          ZSOLQA(JL,NCLDQI,NCLDQI) = ZSOLQA(JL,NCLDQI,NCLDQI)+ZIFG(JL,JK,IPSUPSAT)
          ZPSUPSATSRCE(JL,NCLDQI) = ZIFG(JL,JK,IPSUPSAT)
          ! Add ice to first guess for deposition term 
          ZQXFG(JL,NCLDQI)=ZQXFG(JL,NCLDQI)+ZIFG(JL,JK,IPSUPSAT)
          ! Store cloud budget diagnostics if required
        ENDIF
  ...
  IF (JK < KLEV .AND. JK>=NCLDTOP) THEN
    ...
    DO JL=KIDIA,KFDIA
      ...
      IF(LDCUM(JL).AND.PLUDE(JL,JK) > RLMIN.AND.ZIFG(JL,JK+1,IPLU)> ZEPSEC) THEN
    
        ZSOLAC(JL)=ZSOLAC(JL)+PLUDE(JL,JK)/ZIFG(JL,JK+1,IPLU)
      ...
      IF (LDCUM(JL)) ZSOLQA(JL,NCLDQS,NCLDQS) = ZSOLQA(JL,NCLDQS,NCLDQS) + ZIFG(JL,JK,IPSNDE)*ZDTGDP(JL)
  ...
  IF (JK > NCLDTOP) THEN
    ...
    DO JL=KIDIA,KFDIA
      ZMF(JL)=MAX(0.0_JPRB,(ZIFG(JL,JK,IPMFU)+ZIFG(JL,JK,IPMFD))*ZDTGDP(JL))
      ZACUST(JL)=ZMF(JL)*ZANEWM1(JL)
      ZDTDP=ZRDCP*0.5_JPRB*(ZTP1(JL,JK-1)+ZTP1(JL,JK))/ZIFG(JL,JK,IPAPH)
      ZDTFORC = ZDTDP*(ZIFG(JL,JK,IPAP)-ZIFG(JL,JK-1,IPAP))
...
!          ZEVAP=0.0_JPRB
  ...
  DO JL=KIDIA,KFDIA
    ...
    IF(JK<KLEV) THEN
      ...
      ZMFDN=MAX(0.0_JPRB,(ZIFG(JL,JK+1,IPMFU)+ZIFG(JL,JK+1,IPMFD))*ZDTGDP(JL))
...
!      ZE=ZLDIFDT(JL)*MAX(ZQSMIX(JL,JK)-ZQE,0.0_JPRB)
  ...
  DO JL=KIDIA,KFDIA
    ZDTDP   = ZRDCP*ZTP1(JL,JK)/ZIFG(JL,JK,IPAP)
    ZDPMXDT = ZDP(JL)*ZQTMST
    ZMFDN   = 0.0_JPRB
    IF(JK < KLEV) ZMFDN=ZIFG(JL,JK+1,IPMFU)+ZIFG(JL,JK+1,IPMFD)
    ZWTOT   = ZIFG(JL,JK,IPVERVEL)+0.5_JPRB*RG*(ZIFG(JL,JK,IPMFU)+ZIFG(JL,JK,IPMFD)+ZMFDN)
    ZWTOT   = MIN(ZDPMXDT,MAX(-ZDPMXDT,ZWTOT))
    ZZZDT   = ZIFG(JL,JK,IPHRSW)+ZIFG(JL,JK,IPHRLW)
 ...
 ! call ftrace_region_begin("line 1421")
  ...
  DO idx=1,sidx
    JL = sidxlist(idx)
      ...
      !---------------------------
      ZRHC=RAMID
      ZSIGK=ZIFG(JL,JK,IPAP)/ZIFG(JL,KLEV+1,IPAPH)
 ...
 !call ftrace_region_end("line 1421")
  ...
  DO JL=KIDIA,KFDIA
    ...
    IF (ZTP1(JL,JK)<RTT .AND. ZQXFG(JL,NCLDQL)>RLMIN) THEN  ! T<273K
      ...
      ZBDD=RV*ZTP1(JL,JK)*ZIFG(JL,JK,IPAP)/(2.21_JPRB*ZVPICE)
  ...
  ELSEIF (IDEPICE == 2) THEN
    ...
    DO JL=KIDIA,KFDIA
      ...
      IF (ZTP1(JL,JK)<RTT .AND. ZQXFG(JL,NCLDQL)>RLMIN) THEN  ! T<273K
       ...
       &             ZIFG(JL,JK,IPAP)*RCL_APB3*ZTP1(JL,JK)**3._JPRB
  ...
  DO JL=KIDIA,KFDIA
    ...
    !!!!IF(ZTP1(JL,JK) <= RTT) THEN
      ...
      IF (ZICECLD(JL)>ZEPSEC) THEN
        ...
        IF (LAERICEAUTO) THEN
          ZLCRIT=ZIFG(JL,JK,IPICRIT_AER)
          ! 0.3 = N**0.333 with N=0.027 
          ZZCO=ZZCO*(RNICE/ZIFG(JL,JK,IPNICE))**0.333_JPRB
        ELSE
          ZLCRIT=RLCRITSNOW
        ENDIF
   ...
   IF (ZLIQCLD(JL)>ZEPSEC) THEN
    ...
    IF (IWARMRAIN == 1) THEN
      ...
      IF (LAERLIQAUTOLSP) THEN
        ZLCRIT=ZIFG(JL,JK,IPLCRIT_AER)
        ! 0.3 = N**0.333 with N=125 cm-3 
        ZZCO=ZZCO*(RCCN/ZIFG(JL,JK,IPCCN))**0.333_JPRB
      ELSE
...
!       &ZCOVPTOT(JL)/(MAX(ZA(JL,JK),ZEPSEC))
      ...
      IF (LAERLIQCOLL) THEN 
        ...
        ZCFPR=ZCFPR*(RCCN/ZIFG(JL,JK,IPCCN))**0.333_JPRB
      ENDIF
  ...
  DO JL=KIDIA,KFDIA
    ...
    IF(ZICETOT(JL) > ZEPSEC .AND. ZTP1(JL,JK) > RTT) THEN
      ...
      ZTDMTW0 = ZTP1(JL,JK)-RTT-ZSUBSAT* &
                & (ZTW1+ZTW2*(ZIFG(JL,JK,IPAP)-ZTW3)-ZTW4*(ZTP1(JL,JK)-ZTW5))
      ! Not implicit yet... 
...
!! call ftrace_region_end("line 2018")
 ...
 !! Do idx=1,midx
   !!!   JL=midxlist(idx)
      ...
      ZBETA1 = SQRT(ZIFG(JL,JK,IPAP)/&
       & ZIFG(JL,KLEV+1,IPAPH))/RVRFACTOR*ZPRECLR/&
  ...
  DO idx=1, nidx
     JL = nidxlist(idx)
      ...
      ZEVAP_DENOM = RCL_CDENOM1*ZESATLIQ - RCL_CDENOM2*ZTP1(JL,JK)*ZESATLIQ &
              &+ RCL_CDENOM3*ZTP1(JL,JK)**3._JPRB*ZIFG(JL,JK,IPAP)
  ...
  ELSEIF (IEVAPSNOW == 2) THEN
   ...
   DO JL=KIDIA,KFDIA
    ...
    IF(LLO1) THEN
      ...
      ZAPLUSB   = RCL_APB1*ZVPICE-RCL_APB2*ZVPICE*ZTP1(JL,JK)+ &
     &             ZIFG(JL,JK,IPAP)*RCL_APB3*ZTP1(JL,JK)**3
...
DO JK=1,KLEV+1
  DO JL=KIDIA,KFDIA
    ZOFG(JL,JK,IPFPLSL) = ZPFPLSX(JL,JK,NCLDQR)+ZPFPLSX(JL,JK,NCLDQL)
    ZOFG(JL,JK,IPFPLSN) = ZPFPLSX(JL,JK,NCLDQS)+ZPFPLSX(JL,JK,NCLDQI)
  ENDDO
ENDDO
...
!--------
DO JL=KIDIA,KFDIA
  ZOFG(JL,1,IPFSQLF)  = 0.0_JPRB
  ZOFG(JL,1,IPFSQIF)  = 0.0_JPRB
  ZOFG(JL,1,IPFSQRF)  = 0.0_JPRB
  ZOFG(JL,1,IPFSQSF)  = 0.0_JPRB
  ZOFG(JL,1,IPFCQLNG) = 0.0_JPRB
  ZOFG(JL,1,IPFCQNNG) = 0.0_JPRB
  ZOFG(JL,1,IPFCQRNG) = 0.0_JPRB !rain
  ZOFG(JL,1,IPFCQSNG) = 0.0_JPRB !snow
! fluxes due to turbulence
  ZOFG(JL,1,IPFSQLTUR) = 0.0_JPRB
  ZOFG(JL,1,IPFSQITUR) = 0.0_JPRB
ENDDO

DO JK=1,KLEV
  DO JL=KIDIA,KFDIA

    ZGDPH_R = -ZRG_R*(ZIFG(JL,JK+1,IPAPH)-ZIFG(JL,JK,IPAPH))*ZQTMST
    ZOFG(JL,JK+1,IPFSQLF)  = ZOFG(JL,JK,IPFSQLF)
    ZOFG(JL,JK+1,IPFSQIF)  = ZOFG(JL,JK,IPFSQIF)
    ZOFG(JL,JK+1,IPFSQRF)  = ZOFG(JL,JK,IPFSQLF)
    ZOFG(JL,JK+1,IPFSQSF)  = ZOFG(JL,JK,IPFSQIF)
    ZOFG(JL,JK+1,IPFCQLNG) = ZOFG(JL,JK,IPFCQLNG)
    ZOFG(JL,JK+1,IPFCQNNG) = ZOFG(JL,JK,IPFCQNNG)
    ZOFG(JL,JK+1,IPFCQRNG) = ZOFG(JL,JK,IPFCQLNG)
    ZOFG(JL,JK+1,IPFCQSNG) = ZOFG(JL,JK,IPFCQNNG)
    ZOFG(JL,JK+1,IPFSQLTUR) = ZOFG(JL,JK,IPFSQLTUR)
    ZOFG(JL,JK+1,IPFSQITUR) = ZOFG(JL,JK,IPFSQITUR)

    ZALFAW=ZFOEALFA(JL,JK)

    ! Liquid , LS scheme minus detrainment
    ZOFG(JL,JK+1,IPFSQLF)=ZOFG(JL,JK+1,IPFSQLF)+ &
     &(ZQXN2D(JL,JK,NCLDQL)-ZQX0(JL,JK,NCLDQL)+ZIFG(JL,JK,IPVFL)*PTSPHY-ZALFAW*PLUDE(JL,JK))*ZGDPH_R
    ! liquid, negative numbers
    ZOFG(JL,JK+1,IPFCQLNG)=ZOFG(JL,JK+1,IPFCQLNG)+ZLNEG(JL,JK,NCLDQL)*ZGDPH_R

    ! liquid, vertical diffusion
    ZOFG(JL,JK+1,IPFSQLTUR)=ZOFG(JL,JK+1,IPFSQLTUR)+ZIFG(JL,JK,IPVFL)*PTSPHY*ZGDPH_R

    ! Rain, LS scheme 
    ZOFG(JL,JK+1,IPFSQRF)=ZOFG(JL,JK+1,IPFSQRF)+(ZQXN2D(JL,JK,NCLDQR)-ZQX0(JL,JK,NCLDQR))*ZGDPH_R 
    ! rain, negative numbers
    ZOFG(JL,JK+1,IPFCQRNG)=ZOFG(JL,JK+1,IPFCQRNG)+ZLNEG(JL,JK,NCLDQR)*ZGDPH_R

    ! Ice , LS scheme minus detrainment
    ZOFG(JL,JK+1,IPFSQIF)=ZOFG(JL,JK+1,IPFSQIF)+ &
     & (ZQXN2D(JL,JK,NCLDQI)-ZQX0(JL,JK,NCLDQI)+ZIFG(JL,JK,IPVFI)*PTSPHY-(1.0_JPRB-ZALFAW)*PLUDE(JL,JK))*ZGDPH_R
     ! ice, negative numbers
    ZOFG(JL,JK+1,IPFCQNNG)=ZOFG(JL,JK+1,IPFCQNNG)+ZLNEG(JL,JK,NCLDQI)*ZGDPH_R

    ! ice, vertical diffusion
    ZOFG(JL,JK+1,IPFSQITUR)=ZOFG(JL,JK+1,IPFSQITUR)+ZIFG(JL,JK,IPVFI)*PTSPHY*ZGDPH_R

    ! snow, LS scheme
    ZOFG(JL,JK+1,IPFSQSF)=ZOFG(JL,JK+1,IPFSQSF)+(ZQXN2D(JL,JK,NCLDQS)-ZQX0(JL,JK,NCLDQS))*ZGDPH_R 
    ! snow, negative numbers
    ZOFG(JL,JK+1,IPFCQSNG)=ZOFG(JL,JK+1,IPFCQSNG)+ZLNEG(JL,JK,NCLDQS)*ZGDPH_R
  ENDDO
ENDDO
...
DO JK=1,KLEV+1
  DO JL=KIDIA,KFDIA
    ZOFG(JL,JK,IPFHPSL) = -RLVTT*ZOFG(JL,JK,IPFPLSL)
    ZOFG(JL,JK,IPFHPSN) = -RLSTT*ZOFG(JL,JK,IPFPLSN)
  ENDDO
ENDDO
